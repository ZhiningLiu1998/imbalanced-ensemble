"""Test the module easy ensemble classifier."""
...
def test_easy_ensemble_classifier(estimator, params):
...
def test_gridsearch():
    # Check that easyens ensembles can be grid-searched.
...
def test_warm_start_with_oob_score_fails():
    ...
    X, y = make_hastie_10_2(n_samples=20, random_state=1)
    clf = EasyEnsembleClassifier(n_estimators=5, warm_start=True, oob_score=True)
    with pytest.raises(ValueError):
        clf.fit(X, y)
...
def test_estimators_samples():
    ...
    estimators_samples = easyens.estimators_samples_
    estimators_features = easyens.estimators_features_
    estimators = easyens.estimators_
