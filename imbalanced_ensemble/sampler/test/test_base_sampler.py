...
from imbalanced_ensemble.sampler.base import SamplerMixin, FunctionSampler
...
def test_sampler_mixin(as_frame, sample_weight):
    ...
    if sample_weight is None:
        X_resampled, y_resampled = sampler.fit_resample(
            X_, y, sample_weight=sample_weight,
        )
        if as_frame:
...
def test_func_sampler(sample_weight):
    ...
    X_resampled, y_resampled, w_resampled = \
        func_sampler.fit_resample(
            X, y
        )
