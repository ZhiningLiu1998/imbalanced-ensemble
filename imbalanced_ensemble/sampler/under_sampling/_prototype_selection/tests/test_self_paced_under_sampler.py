...
@pytest.mark.parametrize("invalid_target", [True, False], ids=["invalid_target", "valid_target"])
@pytest.mark.parametrize("replacement", [True, False], ids=["replacement", "non_replacement"])
def test_spus_hard_resample(invalid_target, replacement):
    ...
    if invalid_target and not replacement:
        with pytest.raises(RuntimeError, match="with insufficient number of data samples"):
            X_resampled, y_resampled = spus.fit_resample(
                X, y, 
                alpha=0, 
                y_pred_proba=pred_proba_normal, 
