...
from imbalanced_ensemble.sampler.under_sampling import BalanceCascadeUnderSampler
...
@pytest.mark.parametrize("sampling", ['org', 'auto', 'default', 'normal', 'error'], 
                         ids=['org', 'auto', 'default', 'normal', 'error'])
@pytest.mark.parametrize("keep_pop", ['org','default', 'normal'], 
                         ids=['org', 'default', 'normal'])
...
@pytest.mark.parametrize("sampling", ['org', 'auto', 'default', 'normal'], 
...
def test_bcus_with_sample_weight(sampling):
    ...
    X_resampled, y_resampled, sample_weight_under, new_dropped_index = \
    bcus.fit_resample(
        X, y,
        y_pred_proba=pred_proba_normal, 
