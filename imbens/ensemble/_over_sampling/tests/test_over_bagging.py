...
def test_balanced_bagging_classifier(estimator, params):
    ...
    bag = OverBaggingClassifier(estimator=estimator, random_state=0, **params).fit(
        X_train, y_train
    )
    bag.predict(X_test)
...
def test_single_estimator():
    ...
    clf2 = make_pipeline(
        RandomOverSampler(random_state=clf1.estimators_[0].steps[0][1].random_state),
        KNeighborsClassifier(),
    ).fit(X_train, y_train)
...
def test_gridsearch():
    ...
    GridSearchCV(
        OverBaggingClassifier(SVC(gamma="scale")),
        parameters,
        cv=3,
        scoring="roc_auc",
    ).fit(X, y)
...
def test_estimator():
    ...
    ensemble = OverBaggingClassifier(None, n_jobs=3, random_state=0).fit(
        X_train, y_train
    )
...
def test_bagging_with_pipeline():
    ...
    estimator = OverBaggingClassifier(
...
def test_warm_start(random_state=42):
    ...
    clf_no_ws = OverBaggingClassifier(
...
def test_warm_start_smaller_n_estimators():
    ...
    X, y = make_hastie_10_2(n_samples=20, random_state=1)
    clf = OverBaggingClassifier(n_estimators=5, warm_start=True)
    clf.fit(X, y)
...
def test_warm_start_equal_n_estimators():
    ...
    clf = OverBaggingClassifier(n_estimators=5, warm_start=True, random_state=83)
...
def test_warm_start_equivalence():
    ...
    clf_ws = OverBaggingClassifier(n_estimators=5, warm_start=True, random_state=3141)
    clf_ws.fit(X_train, y_train)
    clf_ws.set_params(n_estimators=10)
    clf_ws.fit(X_train, y_train)
    y1 = clf_ws.predict(X_test)

    clf = OverBaggingClassifier(n_estimators=10, warm_start=False, random_state=3141)
    clf.fit(X_train, y_train)
...
def test_warm_start_with_oob_score_fails():
    ...
    X, y = make_hastie_10_2(n_samples=20, random_state=1)
    clf = OverBaggingClassifier(n_estimators=5, warm_start=True, oob_score=True)
    with pytest.raises(ValueError):
        clf.fit(X, y)
...
def test_oob_score_removed_on_warm_start():
    ...
    clf = OverBaggingClassifier(n_estimators=50, oob_score=True)
    clf.fit(X, y)
...
def test_estimators_samples():
    ...
    # remap the y outside of the OverBaggingClassifier
