...
def test_overboost(imbalanced_dataset, algorithm):
    ...
    overboost.fit(X_train, y_train)
    assert_array_equal(classes, overboost.classes_)

    # check that we have an ensemble of samplers and estimators with a
    # consistent size
    assert len(overboost.estimators_) > 1
    assert len(overboost.estimators_) == len(overboost.samplers_)

    # each sampler in the ensemble should have different random state
    assert len({sampler.random_state for sampler in overboost.samplers_}) == len(
        overboost.samplers_
    )
    # each estimator in the ensemble should have different random state
    assert len({est.random_state for est in overboost.estimators_}) == len(
        overboost.estimators_
    )

    # check the consistency of the feature importances
    assert len(overboost.feature_importances_) == imbalanced_dataset[0].shape[1]

    # check the consistency of the prediction outpus
    y_pred = overboost.predict_proba(X_test)
    assert y_pred.shape[1] == len(classes)
    assert overboost.decision_function(X_test).shape[1] == len(classes)

    score = overboost.score(X_test, y_test)
    assert score > 0.6, f"Failed with algorithm {algorithm} and score {score}"

    y_pred = overboost.predict(X_test)
...
def test_overboost_sample_weight(imbalanced_dataset, algorithm):
    ...
    overboost = OverBoostClassifier(algorithm=algorithm, random_state=0)

    # Predictions should be the same when sample_weight are all ones
    y_pred_sample_weight = overboost.fit(X, y, sample_weight=sample_weight).predict(X)
    y_pred_no_sample_weight = overboost.fit(X, y).predict(X)
