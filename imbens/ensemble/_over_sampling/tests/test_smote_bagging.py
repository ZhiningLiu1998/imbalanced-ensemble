...
def test_gridsearch():
    ...
    GridSearchCV(
        SMOTEBaggingClassifier(SVC(gamma="scale")),
        parameters,
        cv=3,
        scoring="roc_auc",
    ).fit(X, y)
...
def test_estimator():
    ...
    ensemble = SMOTEBaggingClassifier(None, n_jobs=3, random_state=0).fit(
        X_train, y_train
    )
...
def test_bagging_with_pipeline():
    ...
    estimator = SMOTEBaggingClassifier(
...
def test_warm_start_smaller_n_estimators():
    ...
    X, y = make_hastie_10_2(n_samples=20, random_state=1)
    clf = SMOTEBaggingClassifier(n_estimators=5, warm_start=True)
    clf.fit(X, y)
...
def test_warm_start_equal_n_estimators():
    ...
    clf = SMOTEBaggingClassifier(
        k_neighbors=1, n_estimators=5, warm_start=True, random_state=83
    )
...
def test_warm_start_equivalence():
    ...
    clf_ws = SMOTEBaggingClassifier(
        k_neighbors=1, n_estimators=5, warm_start=True, random_state=3141
    )
    ...
    clf = SMOTEBaggingClassifier(
        k_neighbors=1, n_estimators=10, warm_start=False, random_state=3141
    )
    clf.fit(X_train, y_train)
...
def test_warm_start_with_oob_score_fails():
    ...
    X, y = make_hastie_10_2(n_samples=20, random_state=1)
    clf = SMOTEBaggingClassifier(n_estimators=5, warm_start=True, oob_score=True)
    with pytest.raises(ValueError):
        clf.fit(X, y)
...
def test_oob_score_removed_on_warm_start():
    ...
    clf = SMOTEBaggingClassifier(n_estimators=50, oob_score=True)
    clf.fit(X, y)
